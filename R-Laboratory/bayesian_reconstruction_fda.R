...
results <- data.frame()
bk <- 3
short_reconstruction_fda <- data.frame()
...
for(loc in setdiff(unique(data$location), "basement"))
{
  ...
  for(j in 1:23)
    ...
    variables <- data.frame(
      a1 = train_temperature_X,
      b1 = sin(2*pi*(1:23)/22) * train_temperature_X,
      c1 = cos(2*pi*(1:23)/22) * train_temperature_X,
      b2 = sin(2*pi*(1:23)/22),
      c2 = cos(2*pi*(1:23)/22)
    )
    ...
    for(rep in unique(table$name))
    {
      table %>%
        ...
        transmute(Y = value) %>%
        bind_cols(variables) %>%
        lm(Y ~ a1 + b1 + c1 + b2 + c2 + 1, data = .) %>%
        coef() -> coefs_obs
      
      coefs_obs <- replace_na(coefs_obs, 0)
      
      names(coefs_obs)[1] <- "a2"
    ...
    {
      bcfsy1 <- coefsy[i,][c('a1', 'b1', 'c1')]
      bcfsy2 <- coefsy[i,][c('a2', 'b2', 'c2')]
      
      beta1 <- bcfsy1['a1'] + bcfsy1['b1']*sin(2*pi*(1:23)/22) + bcfsy1['c1']*cos(2*pi*(1:23)/22)
      beta2 <- bcfsy2['a2'] + bcfsy2['b2']*sin(2*pi*(1:23)/22) + bcfsy2['c2']*cos(2*pi*(1:23)/22)
      
      recon_fda <- cbind(beta1, beta1, beta1)*test_temperature_X + cbind(beta2, beta2, beta2)
      
      final <- rbind(final,
                     data.frame(ind = i,
                                X = c(1:69),
                                bayes = coefsy[i,7],
                                recon = as.numeric(recon_fda)))
    ...
    likelihood1 <- sum(reconstruction$recon)
    likelihood2 <- sum(old_recon$Y)
    
    final %>%
      group_by(ind) %>%
      summarise(ADD = sum(recon)) %>%
      left_join(
        dplyr::select(bayes_extend, ind = name, bayes_value),
        by = 'ind'
      ) %>%
      filter(ADD == likelihood1) %>%
      pull(bayes_value) -> l1
    ...
    final %>%
      ...
      mutate(ADD_diff = abs(ADD - likelihood2)) %>%
      filter(ADD_diff == min(ADD_diff)) %>%
      pull(bayes_value) -> l2
    
    if(!is.na(likelihood2))
    {
      likelihood_ratio <- rbind(likelihood_ratio,
    ...
    row_of_results <- data.frame(location = loc, 
                                 length = j, 
                                 bayes_error_value_fda = calculated_error,
                                 best_bayes_value_fda = the_best_error,
                                 simple_error_value = calculated_error_simple_recon,
                                 weather_error_value = calculated_error_weather)
    
    short_reconstruction_fda <- rbind(short_reconstruction_fda, 
                                  row_of_results)
...
for(j in c(1, 2, 4, 8, 16, 23))
{
  ...
  row_of_results <- data.frame(location = loc, 
                               length = j, 
                               bayes_error_value_fda = calculated_error,
                               best_bayes_value_fda = calculated_error,
                               simple_error_value = calculated_error_simple_recon,
                               weather_error_value = calculated_error_weather)
...
short_reconstruction_fda %>%
  pivot_longer(bayes_error_value_fda:weather_error_value) %>%
  filter(length != 1) %>%
