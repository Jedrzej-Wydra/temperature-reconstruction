...
for(loc in unique(data$location))
{
  ...
  for(len in 1:11)
  {
    ...
    #squared error
    #calculated_error <- sqrt(mean(((preds - test_temperature_Y)^2)))
    #percentage absoulte error
    calculated_error <- 100*mean(abs(preds - test_temperature_Y)/abs(test_temperature_Y))
    ...
    #squared error
    #simple_calculated_error <- sqrt(mean(((simple_preds - test_temperature_Y)^2)))
    #percentage absoulte error
    simple_calculated_error <- 100*mean(abs(simple_preds - test_temperature_Y)/abs(test_temperature_Y))
    
    #squared error
    #weather_calculated_error <- sqrt(mean(((test_temperature_X - test_temperature_Y)^2)))
    #percentage absoulte error
    weather_calculated_error <- 100*mean(abs(test_temperature_X - test_temperature_Y)/abs(test_temperature_Y))
