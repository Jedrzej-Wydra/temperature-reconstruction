...
for(col in 1:8)
{
  ...
  df1 <- c(df, t.test(data_rows$const, control$const, paired = TRUE)$p.value)
  df2 <- c(df, t.test(data_rows$sin, control$sin, paired = TRUE)$p.value)
  df3 <- c(df, t.test(data_rows$cos, control$cos, paired = TRUE)$p.value)
